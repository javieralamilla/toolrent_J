...
const UpdateDailyRentalRate = () => {
    ...
    const validateAndUpdate = () => {
        if (!editFormData.dailyRentalRate) {
            setError('Por favor completa el campo de tarifa diaria');
            return;
        }

        const dailyRentalRate = parseInt(editFormData.dailyRentalRate);

        if (isNaN(dailyRentalRate)) {
            setError('La tarifa diaria debe ser un número válido');
            return;
        }
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await toolService.updateDailyRentalRate(toolsInventory.id, dailyRentalRate);

            setSuccess('Tarifa diaria de la herramienta actualizada exitosamente');
            ...
            let errorMessage = 'Error al actualizar la tarifa diaria de arriendo de la herramienta';
    ...
    const handleBack = () => {
        ...
        setToolsInventory(null);
        setSearchValue('');
        setEditFormData({ dailyRentalRate: '' });
        setError('');
        setSuccess('');
    };
    ...
    return (
        <Box sx={{ p: 3 }}>
                    ...
                    {searchMode ? (
                        ...
                        <>
                            <Typography
                                ...
                                sx={{ color: '#0A142E', fontWeight: 'bold', mb: 3 }}
                            >
                                Actualizar Tarifa Diaria de Herramienta
                            </Typography>
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                                <TextField
                                    fullWidth
                                    type="number"
                                    label="Tarifa Diaria de Arriendo"
                                    name="dailyRentalRate"
                                    value={editFormData.dailyRentalRate}
                                    onChange={handleEditChange}
                                    required
                                    aria-label="Tarifa diaria de arriendo"
            ...
            >
                ...
                <DialogContent>
                    <Typography sx={{ mt: 2 }}>
                        ¿Está seguro de que desea actualizar la tarifa diaria de arriendo de la herramienta <strong>{toolsInventory?.name}</strong> a <strong>${editFormData.dailyRentalRate}</strong>?
                    </Typography>
                </DialogContent>
                <DialogActions>
                    <Button
...
export default UpdateDailyRentalRate;